...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.guide_via_x_t = guide_via_x_t
    ...
    def p_mean_variance(
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys() and t >= 50:
            if model_kwargs['y']['inpainting_mask'].device != model_output.device:
                model_kwargs['y']['inpainting_mask'] = model_kwargs['y']['inpainting_mask'].to(model_output.device)
            if model_kwargs['y']['inpainted_motion'].device != model_output.device:
                model_kwargs['y']['inpainted_motion'] = model_kwargs['y']['inpainted_motion'].to(model_output.device)
    ...
    def gradients(self, x, t, model, model_kwargs, hint, mask_hint, joint_ids=None, via_x_t=False):
        with torch.enable_grad():
            ...
            joint_pos = smplx_output.joints.view(batch_size, num_frames, -1, 3)[:,:,:n_joints,:]
    ...
    def guide(self, x, t, model=None, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        for _ in range(n_guide_steps):
            loss, grad = self.gradients(x, t, model, model_kwargs, hint, mask_hint, joint_ids, via_x_t=self.guide_via_x_t)
    ...
    def p_sample(
        ...
        if "hint" in model_kwargs['y'].keys():
            #spatial guidance
            out['mean'] = self.guide(out['mean'], t, model=model, model_kwargs=model_kwargs)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if target.shape[1] == 69:
                terms["transl_mse"] = self.masked_l2(target[:,66:69,:,:], model_output[:,66:69,:,:], mask)
            else:
                terms["transl_mse"] = 0. 
            ...
            terms["loss"] = terms["rot_mse"] + terms["transl_mse"] + terms.get('vb', 0.) +\
