...
from tools.project_config_tools import test_room_list, prox_room_list
    ...
    for scene_name in tqdm(used_scene_names):
        ...
        for interaction in tqdm(used_interactions):
            if scene.support_interaction(interaction):
                ...
                for obj_id, combination in enumerate(obj_combinations):
                    obj_inter_save_dir = args.save_dir  / (args.action + '-' + args.interaction.split('-')[-1] + '-' + str(obj_id)) / 'candidates'
                    obj_meshes = []
                    combination_name = []
                    ...
                    for sample_idx in range(args.num_sample):
                        ...
                        if args.visualize:
                            ...
                            export_file = Path(obj_inter_save_dir, base_name)
                            ...
                            base_name = shared_name + '.ply'
                            mesh_path = Path(obj_inter_save_dir, base_name)
                            body_mesh.export(mesh_path)
                            base_name = shared_name + '.pkl'
                            export_file = Path(obj_inter_save_dir, base_name)
                            with open(export_file, 'wb') as f:
                                pickle.dump({'smplx_param': params2numpy(best_sample['smplx_param']), 'action':args.action, 'combination':combination_name}, f)

                    # write smplx results
                    synthesis_results = [sample['smplx_param'] for sample in samples]
                    result_path = Path(obj_inter_save_dir, method + '.pkl')
                ...
                for combination in obj_combinations:
                    ...
                    for sample_idx in range(args.num_sample):
                        ...
                        for try_idx in range(args.num_try):
                            ...
                            if method == 'direct':
                                ...
                                loss, smplx_param, bodies = posa_optimize(smplx_param, contact, x,
                                                                          object_points_scene[:, :, :, :3], scene, body_model, global_body_mesh, args)
                        ...
                        if args.visualize:
                            ...
                            shared_name = method + '_' + mask_type + '_' + str(sample_idx)
                            base_name = shared_name + '.png'
                            ...
                            base_name = shared_name + '.ply'
                            mesh_path = Path(args.save_dir, base_name)
                            body_mesh.export(mesh_path)
                            #export pkl
                            base_name = shared_name + '.pkl'
                            export_file = Path(args.save_dir, base_name)
                    ...
                    # write smplx results
                    synthesis_results = [sample['smplx_param'] for sample in samples]
                    result_path = Path(args.save_dir, method + '_' + mask_type + '.pkl')
                    result_path.parent.mkdir(exist_ok=True, parents=True)
                    print(result_path)
                    with open(result_path, 'wb') as result_file:
                        pickle.dump(synthesis_results, result_file)
...
if __name__ == '__main__':
    ...
    if args.scene_name in prox_room_list: 
        args.action = args.interaction.split('-')[0].split(' ')[0]
        args.save_dir = Path('dataset/PROX_data/proxs/gmd_poses') / args.scene_name
    else:
