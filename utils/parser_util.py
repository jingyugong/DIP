...
def add_model_options(parser):
    ...
    group.add_argument("--controlnet", default=False, action='store_true', help="Use controlnet.")
    group.add_argument("--controlnet_cond_mode", default='both_action_spatial', type=str)
...
def add_data_options(parser):
    ...
    group.add_argument("--dataset", default='humanml', choices=['humanml', 'kit', 'humanact12', 'uestc', 'mixed'], type=str,
...
def add_generate_options(parser):
    ...
    group.add_argument("--inpainting", default=False, action='store_true', help="Inpaint the poset.")
    group.add_argument("--guide_via_x_t", default=False, action='store_true', help="Use x_t to guide as omnicontrol.")
    group.add_argument("--hint_ref_file", default='', type=str,
                       help="Path to a mixed data file")
    group.add_argument("--show_hint_gt", default=False, action='store_true',
                       help="Show the pose in hint file.")
    ...
    group.add_argument("--compile_model", default=False, action='store_true',
                       help="use torch.compile to compile mdm model.")


def add_scene_options(parser):
    group = parser.add_argument_group('scene')
    group.add_argument("--use_scene_diffusion", default=False, action='store_true',
                       help="Utilize Scene-aware Diffusion.")
    group.add_argument("--single_round_demo_id", default='test_room_walk', type=str,
                       help="The demo id for single round motion synthesis in the scene.")
    group.add_argument("--multi_round_demo_id", default='test_room_walk_sit', type=str,
                       help="The demo id for multi round motion synthesis in the scene.")
    group.add_argument("--test_scene_type", default='random_scene_test', type=str,
                       help="The test dataset for motion synthesis in the scene.")
    group.add_argument("--generate_mode", default="demo", choices=["demo", "eval"], type=str,
                       help="Generate for demo or evaluation.")
    group.add_argument("--max_lasting_frames", default=1, type=int, help="Maximum number of frames for the last pose.")
...
def generate_in_scene_args():
    parser = ArgumentParser()
    # args specified by the user: (all other will be loaded from the model)
    add_base_options(parser)
    add_sampling_options(parser)
    add_generate_options(parser)
    add_scene_options(parser)
    args = parse_and_load_from_model(parser)
    cond_mode = get_cond_mode(args)
