...
from tools.demo_tools import single_round_demos
...
def main():
    ...
    demo_data = single_round_demos(args.single_round_demo_id, n_frames)
    hints = demo_data['hints']
    xy_guidance_hints = demo_data['xy_guidance_hints']
    guidance_hints= demo_data['guidance_hints']
    scene_hints = demo_data['scene_hints']
    transf_rotmat = demo_data['transf_rotmat']
    transf_transl = demo_data['transf_transl']
    gender = demo_data['gender']
    init_betas = demo_data['init_betas']
    ...
    if args.controlnet:
        ...
        collate_args = [dict(arg, hint=hint, xy_guidance_hint=xy_guidance_hint, guidance_hint=guidance_hint, scene_hint=scene_hint) for arg, hint, xy_guidance_hint, guidance_hint, scene_hint in zip(collate_args, hints, xy_guidance_hints, guidance_hints, scene_hints)]
    ...
    for rep_i in range(args.num_repetitions):
        ...
        if model.data_rep == 'mixed_vec':
            sample = sample.permute(0, 3, 1, 2).squeeze(-1)
        else:
    ...
    # transform motion from local to global
    local_transls, local_poses = all_motions[:, :, 66:].reshape(-1, 3), all_motions[:, :, :66].reshape(-1, 66)
    transf_inv_rotmat, transf_inv_transl = coordinate_inv_transform(transf_rotmat, transf_transl)
    global_transls, global_poses = motion_coordinate_transform(gender, init_betas, local_transls, local_poses, transf_inv_rotmat, transf_inv_transl)
    all_motions = np.concatenate([global_poses, global_transls], axis=1).reshape(*all_motions.shape)
