...
from tools.agent_tools import MultiRoundMotionAgent
...
from tools.demo_tools import multi_round_demos
from tools.demo_candidates_tools import generate_multi_round_demo_candidates
...
def main():
    ...
    fixseed(args.seed)
    out_path = os.path.join(args.output_dir, args.multi_round_demo_id)
    ...
    if args.generate_mode == "demo":
        demo_data = multi_round_demos(args.multi_round_demo_id)
    elif args.generate_mode == "eval":
        demo_data = generate_multi_round_demo_candidates(args.multi_round_demo_id)
    else:
        raise NotImplementedError

    multi_round_motion_agent = MultiRoundMotionAgent(
        demo_data,
        model,
        diffusion,
        data,
        ...
        mdm_guidance_param=args.guidance_param, #mdm default parameter
        mdm_batch_size=args.batch_size, #mdm default parameter
    ...
    for rep_i in range(args.num_repetitions):
        ...
        all_text.append(results['sample_text'])
        all_motions.append(results['sample'])
        all_lengths.append(results['sample_lengths'])
