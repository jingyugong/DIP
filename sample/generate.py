...
def main():
    ...
    if is_using_data:
        ...
        if is_t2m:
            ...
            if args.controlnet and args.hint_ref_file != '':
                hints = [load_random_hint_from_file(args.hint_ref_file)]
                hint_gt = load_feature69dim_from_file(args.hint_ref_file)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        if not is_using_data and not is_t2m and args.controlnet and args.hint_ref_file != '' and args.show_hint_gt:
            sample = torch.as_tensor(hint_gt, device=dist_util.dev()).T.unsqueeze(0).unsqueeze(2)
        else:
            sample = sample_fn(
                model,
